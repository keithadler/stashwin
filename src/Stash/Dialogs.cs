...
public sealed class RecoveryCardWindow : Window
{
    ...
    public RecoveryCardWindow(Shell shell, MasterKey key)
    {
        ...
        Title = "Your recovery card"; Width = 640; SizeToContent = SizeToContent.Height; ResizeMode = ResizeMode.NoResize; WindowStartupLocation = WindowStartupLocation.CenterOwner; Background = Ui.B("Ground");
        ...
        for (int r = 0; r < 6; r++) grid.RowDefinitions.Add(new RowDefinition());
        var words = key.Words;
        for (int i = 0; i < 24; i++)
        {
            var t = new TextBlock { Margin = new Thickness(0, 3, 0, 3), FontFamily = new FontFamily("Cascadia Mono, Consolas"), FontSize = 14 };
            t.Inlines.Add(new Run($"{i + 1,2}. ") { Foreground = Ui.B("Muted") });
            t.Inlines.Add(new Run(words[i]) { FontWeight = FontWeights.SemiBold });
            Grid.SetRow(t, i % 6); Grid.SetColumn(t, i / 6);
            grid.Children.Add(t);
        }
        var inner = new StackPanel();
        inner.Children.Add(grid);
        inner.Children.Add(Ui.Text($"Key fingerprint {key.Fingerprint}. Two cards can be told apart by it.", "Small", new Thickness(0, 10, 0, 0)));
        card.Child = inner;
    ...
    private void Print()
    {
        ...
        var doc = new FlowDocument { PagePadding = new Thickness(60), FontFamily = new FontFamily("Cascadia Mono, Consolas"), FontSize = 13 };
        doc.Blocks.Add(new Paragraph(new Run("Stash recovery card")) { FontSize = 22, FontWeight = FontWeights.Bold });
        doc.Blocks.Add(new Paragraph(new Run("These 24 words are the only way to read the backup. Keep this somewhere safe; treat it like a passport.")));
        doc.Blocks.Add(new Paragraph(new Run(Mnemonic.Card(_key.Words))) { FontSize = 15 });
        doc.Blocks.Add(new Paragraph(new Run($"Fingerprint {_key.Fingerprint}\nRestore with Stash for Windows (github.com/keithadler/stashwin) or Stash for Mac (github.com/keithadler/stashmac).")));
        dlg.PrintDocument(((IDocumentPaginatorSource)doc).DocumentPaginator, "Stash recovery card");
    }
}
...
public sealed class EnterCardWindow : Window
{
    ...
    {
        Title = "Enter a recovery card"; Width = 560; SizeToContent = SizeToContent.Height; ResizeMode = ResizeMode.NoResize; WindowStartupLocation = WindowStartupLocation.CenterOwner; Background = Ui.B("Ground");
        SourceInitialized += (_, _) => Theme.DecorateTitleBar(this);
        var panel = new StackPanel { Margin = new Thickness(28, 22, 28, 20) };
...
public sealed class RestoreWindow : Window
{
    ...
    public RestoreWindow(Shell shell, SnapshotRow snap)
    {
        ...
        Title = "Restore"; Width = 760; Height = 620; WindowStartupLocation = WindowStartupLocation.CenterOwner; Background = Ui.B("Ground");
        SourceInitialized += (_, _) => Theme.DecorateTitleBar(this);
        ...
        head.Children.Add(Ui.Text($"Restore from {snap.When}", "H1"));
        ...
        left.Children.Add(Ui.Button("Choose folder…", (_, _) => { var d = new OpenFolderDialog { Title = "Restore into" }; if (d.ShowDialog(this) == true) { _targetPath = d.FolderName; _target.Text = "Where: " + _targetPath; } }));
    ...
    {
        if (_targetPath is null) { MessageBox.Show(this, "Choose where the files should go first.", "Stash for Windows"); return; }
        ...
        if (!everything && only.Count == 0) { MessageBox.Show(this, "Select some files, or restore everything.", "Stash for Windows"); return; }
        IsEnabled = false;
...
public sealed class SettingsWindow : Window
{
    ...
    {
        Title = "Settings"; Width = 560; SizeToContent = SizeToContent.Height; ResizeMode = ResizeMode.NoResize; WindowStartupLocation = WindowStartupLocation.CenterOwner; Background = Ui.B("Ground");
        SourceInitialized += (_, _) => Theme.DecorateTitleBar(this);
        ...
        foreach (var (v, label) in new[] { ("off", "Only when I press Back Up Now"), ("hourly", "Every hour"), ("daily", "Once a day, at 20:00") }) schedule.Items.Add(new ComboBoxItem { Content = label, Tag = v });
        ...
        var last = new RadioButton { Content = "The newest", IsChecked = cfg.Retention != "thin", Margin = new Thickness(0, 0, 8, 0), VerticalContentAlignment = VerticalAlignment.Center, Foreground = Ui.B("Ink") };
        var keep = new TextBox { Width = 60, Text = cfg.KeepSnapshots.ToString() };
        var thin = new RadioButton { Content = "Thin out over time: every one from the last week, one a day for a month, one a week for a year, one a month after that", IsChecked = cfg.Retention == "thin", Margin = new Thickness(0, 6, 0, 0), Foreground = Ui.B("Ink") };
        ...
        var weekly = new CheckBox { Content = "Check the backup once a week: open every piece and restore one random file", IsChecked = cfg.WeeklyVerify, Margin = new Thickness(0, 14, 0, 0) };
...
public sealed class AboutDialog : Window
{
    public AboutDialog()
    {
        Title = "About Stash for Windows"; Width = 520; SizeToContent = SizeToContent.Height; ResizeMode = ResizeMode.NoResize; WindowStartupLocation = WindowStartupLocation.CenterOwner; Background = Ui.B("Ground");
        SourceInitialized += (_, _) => Theme.DecorateTitleBar(this);
        ...
        titles.Children.Add(Ui.Text("Version " + Cli.Version + ", MIT licence", "Small"));
        ...
        var src = new Hyperlink(new Run("Source and releases")) { NavigateUri = new Uri("https://github.com/keithadler/stashwin") };
        var more = new Hyperlink(new Run("More from the same maker")) { NavigateUri = new Uri("https://keithadler.github.io/") };
